...
schedules_on_date_collection = mongo_db[f'sched_on_day_{readable_date_of_tt}']
schedules_on_previous_date_collection = mongo_db[f'sched_previous_day_{readable_date_of_tt}']
...
def parse_cif_file(filename: str, date_of_tt: str, **kwargs):
    ...
    import_update_atoc_schedule = 'import_update_atoc_schedule' in kwargs and kwargs[
        'import_update_atoc_schedule'] is True
    ...
    for line in f:
        if line[0:2] == 'HD':  # Header record
            ...
            if date_runs > 0 and (import_full_atoc_schedule is True or import_update_atoc_schedule is True) and len(
                    current_schedule) > 0:
